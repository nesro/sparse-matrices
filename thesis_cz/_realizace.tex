...
Popsané algoritmy jsme implementovali v jazyce C99. Implementace probíhala v operačním systému Lubuntu, v IDE Eclipse CDT. Zdrojový kód byl kompilován pomocí překladače GNU C.

K lazení chyb pro práci s pamětí jsme použili nástroj Valgrind s přepínači \texttt{- - leak-check=full}. Běžící progam jsme krokovali pomocí nástoje GNU Debugger.
...
Výsledný program načte dvě matice v určitém formátu a vynásobí je. Pomocí přepínačů lze zobrazit, nebo uložit výslednou matici, popřípadě vypsat některé údaje o načtených maticích.
